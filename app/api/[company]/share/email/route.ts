...
export async function POST(_req: Request, { params }: { params: Promise<{ company: string }> }) {
  try {
    await params; // company context available if needed for branding
    ...
    return NextResponse.json(
      { error: 'FAILED_TO_SEND', message: e instanceof Error ? e.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
