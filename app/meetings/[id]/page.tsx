...
export default function MeetingPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/meetings/${id}`)
      .then(async (r) => {
        ...
        if (!r.ok || data?.error) {
          throw new Error(data?.error || `Failed to fetch meeting (${r.status})`);
        }
  ...
  if (!meeting) {
    return (
      <div className="text-center py-20">
        ...
        <p className="text-gray-500 mt-2">Please check the database connection and refresh the page.</p>
        <Link href="/" className="text-blue-600 mt-4 inline-block">Back to Meetings</Link>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="flex border-b border-gray-200 mb-6">
        <button
          ...
          className={`px-6 py-3 font-semibold text-sm border-b-2 transition-colors ${
            tab === 'agenda' ? 'border-blue-900 text-blue-900' : 'border-transparent text-gray-500 hover:text-gray-700'
          }`}
        >
          Agenda
        </button>
        <button
          ...
          className={`px-6 py-3 font-semibold text-sm border-b-2 transition-colors ${
            tab === 'minutes' ? 'border-blue-900 text-blue-900' : 'border-transparent text-gray-500 hover:text-gray-700'
          }`}
        >
          Minutes
        </button>
      ...
      {tab === 'agenda' && <AgendaTab key={meeting._id} meeting={meeting} />}
      {tab === 'minutes' && <MinutesTab key={meeting._id} meeting={meeting} />}
    </div>
  );
}
