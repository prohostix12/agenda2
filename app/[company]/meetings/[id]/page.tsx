...
export default function MeetingPage() {
  ...
  if (!meeting) return (
    <div className="text-center py-20">
      ...
      <Link href={`/${company}`} className="text-blue-600 mt-4 inline-block">← Back to Meetings</Link>
    </div>
  );
  ...
  return (
    <div>
      <div className="mb-6">
        <Link href={`/${company}`} className="text-blue-600 hover:text-blue-800 text-sm flex items-center gap-1 mb-4">
      ...
      {tab === 'agenda' && <AgendaTab key={meeting._id} meeting={meeting} companySlug={company} />}
      {tab === 'minutes' && <MinutesTab key={meeting._id} meeting={meeting} companySlug={company} />}
    </div>
  );
}
